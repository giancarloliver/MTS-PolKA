...
def topology(remote_controller):
    ...
    for i in range(0, n_switches_C_N1):  # Add two level-1 switches
        ...
        json_file = path + "/p4src/wcmp.p4.json"
        config =   path + "/sw-commands/s1_{}-commands.txt".format(i)  # Update with the correct path
    ...
    for i in range(0, n_switches_C_N2):  # Add five level-2 switches      
        path = os.path.dirname(os.path.abspath(__file__))
        json_file = path + "/p4src/wcmp.p4.json"
        config =   path + "/sw-commands/s2_{}-commands.txt".format(i)  # Update with the correct path
    ...
    for i in range(1, n_switches_E + 1):
        ...
        path = os.path.dirname(os.path.abspath(__file__))
        json_file = path + "/p4src/wcmp.p4.json"
        config = path + "/sw-commands/e{}-commands.txt".format(i)
