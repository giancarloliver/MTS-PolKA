...
if __name__ == "__main__":
    ...
    try:
        ...
        for test_number in range(1, 31):
            ...
            print("Running Test 4 Perfil 11")

            print("Alterando E1")
            command = 'echo "table_modify tunnel_encap_process_sr add_sourcerouting_header 1 2 1 00:00:00:00:02:02 73817044396459291349659850249 16688267818192630013069185667" | simple_switch_CLI --thrift-port 50101'
            print(f"Executing: {command}")
            subprocess.run(command, shell=True)
            time.sleep(1)

            print("Alterando E2")
            command = 'echo "table_modify tunnel_encap_process_sr add_sourcerouting_header 1 2 1 00:00:00:00:01:01 6072614871712978375441543038 18446848090728087715511153268" | simple_switch_CLI --thrift-port 50102'
            print(f"Executing: {command}")
            subprocess.run(command, shell=True)
            time.sleep(1)
            ...
            # Verifique se o servidor iperf está em execução
            server_check = h2.cmd('pgrep -f "iperf3 -s"')
            if not server_check:
                print("Error: iperf server not running on h2")
                continue

            # Iniciando o iperf cliente
            output_file = f'data/test4/output-P11_{test_number}.txt'
            ...
            iperf_cmd = f'iperf3 -c {h2.IP()} -n 10M --verbose > {output_file} 2>&1'
