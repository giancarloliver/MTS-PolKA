...
def process_files(output_dir, profile_name):
    ...
    if retransmissions:
        ...
        return average_retransmissions
    else:
        return None
...
avg_retrans_profile0 = process_files(output_dir, 'perfil0')

# Process data for profile 3
avg_retrans_profile3 = process_files(output_dir, 'perfil3')

# Definir desvios padrão específicos
specified_deviations = [0.0, 0.5, 1.0, 1.5, 2.0]
...
if avg_retrans_profile0 is not None and avg_retrans_profile3 is not None:
    ...
    # Plot profile 0 data
    plt.errorbar(time_points, [avg_retrans_profile0] * len(time_points), yerr=specified_deviations[:len(time_points)], fmt='o', linestyle='-', color='b', label='Perfil 0')
    
    # Plot profile 3 data
    plt.errorbar(time_points, [avg_retrans_profile3] * len(time_points), yerr=specified_deviations[:len(time_points)], fmt='o', linestyle='-', color='r', label='Perfil 3')
    
    plt.axhline(y=0, color='black', linestyle='--')
    plt.xlabel('Tempo (segundos)')
    plt.ylabel('Retransmissões com Desvio')
    plt.title('Média de Retransmissões com Desvios Específicos ao Longo do Tempo')
