...
if __name__ == "__main__":
    ...
    for j in range(samples):
        ...
        print(f"1 tcp flow 1 with origin H1(connected to s1_0) and dest H2(connected to s1_1) with traffic profile 0")
        h1.cmd(f'iperf3 -c {h2.IP()} -t 22 -b -w {tcp_window_size} &')

        print(f"1 tcp flow 2 with origin H1(connected in s1_0) and dest H5(connected in s1_1) with traffic profile 1")
        h1.cmd(f'iperf3 -c {h5.IP()} -t 22 -b -w {tcp_window_size} &')
        # tcp_window_size = "1M" 
        # h1.cmd(f'iperf3 -c {h2.IP()} -t 30 &')
        # h1.cmd(f'iperf3 -c {h5.IP()} -t 30 &')
       
        print(f"1 udp flow with origin H3(connected to s2_2) and dest H4(connected to s1_1) with traffic profile 1")
        iperf_cmd = f'iperf3 -u -c {h4.IP()} -t 22 -b 4M'
        h3.cmd(iperf_cmd + ' &')   

        ### Inciando captura dos dados com bwm-ng
        print(f"Start bwm-ng")
        monitor_cpu.start()
        time.sleep(1)  
                              
               
        # aguardar 10s
        print(f"Wait for initial test for 10 seconds")
        time.sleep(10)

        # Modify the P4 table with profile information test 3 1-1
        print(f"Migration: tcp flow with origin H1 and dest H2 changes to 1:1 traffic profile passing through S2_0 and s2_1")
        ...
        # aguardar 20s
        print(f"Wait for initial test for 25 seconds")
        time.sleep(22)
