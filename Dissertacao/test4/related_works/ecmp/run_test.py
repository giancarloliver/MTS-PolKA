...
def create_data_directories():
    ...
    directories = [
        ...
        "fct_test/data/run/data/run",
        "fct_test/data/run/result"
    ]
...
if __name__ == "__main__":
    ...
    for _ in range(samples):
        ...
        h1.cmd(f'iperf3 -c {h9.IP()} -n 100M --verbose &> fct_test/data/run/{samples}-iperf_h1_h9.txt &')

        h3.cmd(f'iperf3 -c {h11.IP()} -n 100M --verbose &> fct_test/data/run/{samples}-iperf_h3_h11.txt &')
        ...
        #os.system(f"cat fct_test/data/run6/{test}-tmp.bwm >> {bwm_file}")
