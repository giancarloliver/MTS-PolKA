...
def topology(remote_controller):
    ...
    for i in range(0, n_switches_C_N1):  # Add two level-1 switches
        ...
        json_file = f"{path}/p4src/rps.json"
    ...
    net.addLink(hosts[10], edges[0], bw=BWE, delay=D)
    ...
    net.addLink(hosts[14], edges[3], bw=BWE, delay=D)
    net.addLink(hosts[15], edges[3], bw=BWE, delay=D)

    net.addLink(hosts[16], edges[4], bw=BWE, delay=D)
    net.addLink(hosts[17], edges[5], bw=BWE, delay=D)
...
if __name__ == "__main__":
    ...
    h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, h15, h16, h17, h18 = net.get('h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'h7', 'h8', 'h9', 'h10', 'h11', 'h12', 'h13', 'h14', 'h15', 'h16', 'h17', 'h18')    
    ...
    for _ in range(samples):
        ...
        time.sleep(1)
         ...
         #Testing with 4 elephant flows - iperf3              
        ...
        lambda_rate = 1 # Average rate of flow initiation (flows per second)
        duration = 400  # Duration of the experiment (seconds)
        flow_size_kb = 1 # Fixed size of each flow (in Kilobytes)

        #latency test

        h7.cmd(f'ping {h12.IP()} > latency_test/data/run/{method}_path1.log &')
        h8.cmd(f'ping {h13.IP()} > latency_test/data/run/{method}_path2.log &')        
        h9.cmd(f'ping {h14.IP()} > latency_test/data/run/{method}_path3.log &')
        h10.cmd(f'ping {h15.IP()} > latency_test/data/run/{method}_path4.log &')
        h11.cmd(f'ping {h16.IP()} > latency_test/data/run/{method}_path5.log &')
